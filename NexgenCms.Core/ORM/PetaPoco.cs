...
namespace PetaPoco {
    ...
    public class Database : IDisposable {
        ...
        private void CommonConstruct() {
            ...
            //else if (dbtype.StartsWith("Db2")) _dbType = DBType.Db2;
            ...
            //else if (_providerName.IndexOf("Db2", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.Db2;
        ...
        //public DatabaseConnectionModel ToDatabaseConnectionModel(object ) {
        ...
        public IEnumerable<T> QueryAll<T>() {
            ...
            try {
                ...
                var pd = PocoData.ForType(typeof(T));
                using (var cmd = CreateCommand(_sharedConnection, "SELECT * FROM " + pd.TableInfo.TableName, null)) {
                    IDataReader r;
                    try {
                        cmd.CommandTimeout = 0;
                        r = cmd.ExecuteReader();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public IEnumerable<T> QueryByID<T>(int id) {
            OpenSharedConnection();
            try {

                var pd = PocoData.ForType(typeof(T));
                using (var cmd = CreateCommand(_sharedConnection, "SELECT * FROM " + pd.TableInfo.TableName + " WHERE " + pd.TableInfo.PrimaryKey + " = " + id + ";", null)) {
                    IDataReader r;
                    try {
                        cmd.CommandTimeout = 0;
                        r = cmd.ExecuteReader();
        ...
        public string FormatCommand(string sql, object[] args) {
            ...
            if (args != null && args.Length > 0) {
                ...
                for (int i = 0; i < args.Length; i++) {
                    sb.AppendFormat("\t -> {0}{1} [{2}] = \"{3}\"\n", _paramPrefix, i, args[i].GetType().Name, args[i] != null ? args[i] : "");
                }
                ...
                try {
                    ...
                    if (type == typeof(object)) {
                        ...
                        if (type.IsValueType || type == typeof(string) || type == typeof(byte[])) {
                            ...
                            if (ctor == null)
                                throw new ApplicationException("Type [" + type.FullName + "] should have default public or non-public constructor");
                    ...
                    if (dstType.IsEnum && IsIntegralType(srcType)) {
                        ...
                        if (dstType.IsGenericType && dstType.GetGenericTypeDefinition().Equals(typeof(Nullable<>))) {
                            converter = delegate (object src) {
                                return Convert.ChangeType(src, Nullable.GetUnderlyingType(dstType), null);
                            };
                        } else {
                            converter = delegate (object src) {
